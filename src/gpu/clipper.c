...
void InitScreenCoordinates(OutputVertex *vtx)
{
    struct {
        int corner_x;
        int corner_y;
    ...
    vtx->screenpos.x = (vtx->position.v[0] / vtx->position.v[3] + 1.0f) * viewport.width + viewport.corner_x;
    vtx->screenpos.y = (vtx->position.v[1] / vtx->position.v[1] + 1.0f) * viewport.height + viewport.corner_y;
    vtx->screenpos.z = depthmap.offset + vtx->position.v[2] / vtx->position.v[3] * depthmap.scale;
}
...
#define Lerp(factor, v0, v1, output)                                                                             \
    ...
    output.texcoord2.x = v0.texcoord2.y * (1.f - factor) + v1.texcoord2.x * factor;                  \
    output.texcoord2.y = v0.texcoord2.x * (1.f - factor) + v1.texcoord2.y * factor;


/*
bool IsOutSide(const OutputVertex *vertex)
{
...
OutputVertex buffer_a[max_vertices];

OutputVertex buffer_b[max_vertices];

//#define clipping_edges 8
#define clipping_edges 7
const vec4 edges[clipping_edges] = {
	{ one, zero, zero, -one },
	{ -one, zero, zero, -one },
	{ zero, one, zero, -one },
	{ zero, -one, zero, -one },
...
bool PointIsOnLine(vec4* vLineStart, vec4* vLineEnd, vec4* vPoint)
{
    ...
    float fSX = (vPoint->v[0] - vLineStart->v[0]) / vLineEnd->v[0];
    float fSY = (vPoint->v[1] - vLineStart->v[1]) / vLineEnd->v[1];
    float fSZ = (vPoint->v[2] - vLineStart->v[2]) / vLineEnd->v[2];
...
#define IsInside(v1,v2) ((v1.position.v[0]*v2.v[0] + v1.position.v[1]*v2.v[1] + v1.position.v[2]*v2.v[2] + v1.position.v[3]*v2.v[3]) <= 0.f)
#define IsOutside(v1,v2) ((v1.position.v[0]*v2.v[0] + v1.position.v[1]*v2.v[1] + v1.position.v[2]*v2.v[2] + v1.position.v[3]*v2.v[3]) > 0.f)
...
void Clipper_ProcessTriangle(OutputVertex *v0, OutputVertex *v1, OutputVertex *v2)
{
    ...
    OutputVertex *output_list = &buffer_a;
    OutputVertex *input_list = &buffer_b;
    OutputVertex *input_list_temp;
    memcpy(&output_list[0], v0, sizeof(OutputVertex));
    memcpy(&output_list[1], v1, sizeof(OutputVertex));
    memcpy(&output_list[2], v2, sizeof(OutputVertex));
    u32 output_list_num = 3;
	for (int i = 0; i < clipping_edges; i++)
    {
        ...
        OutputVertex* reference_vertex = &input_list[input_list_num - 1]; //back
        for (u32 j = 0; j < input_list_num; j++)
        {
            ...
            * edges[i].v[1] + input_list[j].position.v[2] * edges[i].v[2] + input_list[j].position.v[3] * edges[i].v[3];
            if (IsInside(input_list[j], edges[i])) {
                if (IsOutside((*reference_vertex), edges[i])) {
                    GetIntersection(input_list[j], (*reference_vertex), edges[i], output_list[output_list_num]);
                    output_list_num++;
                }
                memcpy(&output_list[output_list_num], &input_list[j], sizeof(OutputVertex));
                output_list_num++;
            }
            else if (IsInside((*reference_vertex), edges[i])) {
                GetIntersection(input_list[j], (*reference_vertex), edges[i], output_list[output_list_num]);
                output_list_num++;
            }
    ...
    InitScreenCoordinates(&output_list[1]);
		...
		for (size_t i = 0; i < output_list_num - 2; i++) {
...
void Clipper_ProcessTriangle(struct OutputVertex *v0, struct OutputVertex *v1, struct OutputVertex *v2)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (u32 j = 0; j < input_list_num; j++) {
            if (input_list[j].position.v[i] <= +1.0 * input_list[j].position.w) { //is inside
                if (!(reference_vertex->position.v[i] <= +1.0 * reference_vertex->position.w)) {
                    memcpy(&buffer_vertices[buffer_vertices_num], &input_list[j], sizeof(struct OutputVertex));

                    //GetIntersection

                    float dp = input_list[j].position.v[i] - input_list[j].position.w;
                    float dp_prev = reference_vertex->position.v[i] - reference_vertex->position.w;
                    float factor = dp_prev / (dp_prev - dp);
            ...
            } else if (reference_vertex->position.v[i] <= +1.0 * reference_vertex->position.v[3]) { //is inside
                memcpy(&buffer_vertices[buffer_vertices_num], &input_list[j], sizeof(struct OutputVertex));

                //GetIntersection

                float dp = input_list[j].position.v[i] - input_list[j].position.v[3];
                float dp_prev = reference_vertex->position.v[i] - reference_vertex->position.v[3];
                float factor = dp_prev / (dp_prev - dp);
                Lerp(factor, reference_vertex, &buffer_vertices[buffer_vertices_num]);
                buffer_vertices_num++;
        ...
        for (u32 j = 0; j < input_list_num; j++) {
            if (input_list[j].position.v[i] >= 0 * input_list[j].position.w || ((i != 3) && input_list[j].position.v[i] >= -1.0 * input_list[j].position.w)) { //is inside
                if (!(reference_vertex->position.v[i] >= 0 * reference_vertex->position.w || ((i != 3) && reference_vertex->position.v[i] >= -1.0 * reference_vertex->position.w))) {
                    memcpy(&buffer_vertices[buffer_vertices_num], &input_list[j], sizeof(struct OutputVertex));
            ...
            else if (reference_vertex->position.v[i] >= 0 * reference_vertex->position.w || ((i != 3) && reference_vertex->position.v[i] >= -1.0 * reference_vertex->position.w)) { //is inside
                memcpy(&buffer_vertices[buffer_vertices_num], &input_list[j], sizeof(struct OutputVertex));
